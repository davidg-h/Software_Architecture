		...
		api.getMeals(openMensaDateFormat.format(new Date())).enqueue(new Callback<>() {
			...
			public void onResponse(Call<List<Meal>> call, Response<List<Meal>> response) {
				...
				Platform.runLater(() -> {
					...
					String strategy = filterChoiceBox.getValue() != null ? filterChoiceBox.getValue() : "";

					MealsFilter mealsFilter = MealsFilterFactory.getStrategy(strategy);

					List<Meal> result = mealsFilter.filter(response.body());
